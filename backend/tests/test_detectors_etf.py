...
def _patch(total: float, raise_error: bool = False):
    ...
    return (
        ...
        patch("backend.agent.detectors.etf_flow_spike.fetch_btc_eth_prices", new=price_mock),
    )


async def test_buy_signal_above_400m(detector):
    gc, etf, prices = _patch(541_000_000)
    with gc, etf, prices:
...
async def test_watch_signal_between_50m_and_400m(detector):
    gc, etf, prices = _patch(200_000_000)
    with gc, etf, prices:
        signals = await detector.run()
    assert len(signals) == 1
...
async def test_avoid_signal_below_neg200m(detector):
    gc, etf, prices = _patch(-250_000_000)
    with gc, etf, prices:
        signals = await detector.run()
    assert len(signals) == 1
...
async def test_watch_signal_moderate_negative(detector):
    gc, etf, prices = _patch(-100_000_000)
    with gc, etf, prices:
        signals = await detector.run()
    assert len(signals) == 1
    assert signals[0]["type"] == "WATCH"
...
async def test_no_signal_within_normal_range(detector):
    gc, etf, prices = _patch(30_000_000)
    with gc, etf, prices:
        signals = await detector.run()
    assert signals == []
...
async def test_signal_has_required_fields(detector):
    gc, etf, prices = _patch(500_000_000)
    with gc, etf, prices:
        signals = await detector.run()
    sig = signals[0]
    for field in (
...
async def test_fetch_error_returns_empty(detector):
    gc, etf, prices = _patch(0, raise_error=True)
    with gc, etf, prices:
        signals = await detector.run()
    assert signals == []
